...
def checkdiff(Vout, Iout, expFile):
    ...
    s = 0
    for i in Vexp.keys():
        #print(f"Vexp[{i}] = {Vexp[i]}", f"Vout[{i}] = {Vout[i]}")
        s += abs(Vexp[i] - Vout[i])
    for i in Iexp.keys():
...
test_spice('test_4.ckt', 'test_4.exp')
test_spice('test_3.ckt', 'test_3.exp')
test_spice('test_2.ckt', 'test_2.exp')
test_spice('test_1.ckt', 'test_1.exp')
test_malformed()
test_invalid_file()
test_voltage_loop()
